---
title: "Prediction of Benign and Malignant Breast Cancer"
...
The goal of this project is to determine whether a breast mass tumor is malignant or benign based on several different physical factors. 

Classes and Variables 
This dataset consists of 3 classes:
- means, standard error (SE) and worst (i.e., mean of three largest values in each variable) 
and 32 variables:
...
Project Methodology:
In addition to predicting the classification of breast tumor, we wanted to find the most effective variable in determining malignancy or benignity of the tumor. This is how we did it: 
- Utilized two predictive modelling techniques: logistic regression (baseline) and neural networks
We settled on 4 logistic regression models and 4 neural networks testing the different classes: All variables, Mean Variables, Worst variables + SE variables 

Our model parameters were: 
Logistic Regression
Train-test split: 70:30
Neural Networks
Number = 3; 3 resampling iterations 
Grid size 2-50 with 1 layer in between
Decay rate = 5e-4 

Below chunk of code consists of: Data cleaning, doing 70-30 split, running logistic regression against ALL variables (mean + se + worst) and performing prediction on test dataset 
```{r}
rm(list=ls())
library(plyr) 
library(dplyr) 
...
Below chunk of code consists of: Logistic regression on ALL MEAN variables and performing test prediction on dataset 
```{r}
...
Below chunk of code consists of: Logistic regression on ALL WORST variables and performing test prediction on dataset 
```{r}
...
Below chunk of code consists of: Logistic regression on ALL MEAN + SE variables and performing test prediction on dataset 
```{r}
...
Below chunk of code consists of: Creating forestplot for logistic regression 
The AUC's and 95% CI for each of the classes are very narrow and it is difficult to identify the best AUC value and the tightest 95% CI, so below is a forest plot to clearly show and compare between the classes. 
Taking a look at this plot, we can note that class All mean + SE has the best AUC and a tight 95% CI therefore being the best logistic regression model out of the 4. 
...
Below chunk of code consists of: Neural Network for ALL variables 
```{r}
library(nnet)
...
Below chunk of code consists of: Neural Network for all MEANS
```{r}
#Create model on training data 
...
Below chunk of code consists of: Neural Network for all WORST
```{r}
#Create model on training data 
...
Below chunk of code consists of: Neural Network for MEANS + SE 
```{r}
#Create model on training data 
...
Below chunk of code consists of: Creating forestplot for Neural Networks
The AUC's and 95% CI for each of the classes are very narrow and it is difficult to identify the best AUC value and the tightest 95% CI, so below is a forest plot to clearly show and compare between the classes. 
Taking a look at this plot, we can note that class All has the best AUC and a tight 95% CI therefore being the best neural network model out of the 4. 
```{r}
library(ggplot2)
...
Below chunk of code consists of: Variable Importance Plot for ALL Neural Networks (the best model)
Comparing the values and the plots, we've confirmed that the best model (between logistic regression and neural networks) is All neural networks. Going one step further, we wanted to see which variable within this All neural network model would best determine the benign/malignant outcome of the tumor. To determine this, we created a variable importance plot as shown below.

As we observe, area_se, radius_worst and perimeter_mean are the most effective variables, greatest to least, respectively. 
```{r}
library(randomForest)
library(ggplot2)
library(nnet)
...
Below chunk of code consists of: Combining ForestPlot for both Logistic Regression and Neural Networks
The forestplot here shows both individual forest plots and we can clearly see the All Neural Network has the highest AUC and the tightest 95%CI out of all the logistic regression and neural network models, therefore being the best at predicting malignancy/benignity of breast cancer tumors.  
...
Table 1 Data
The P-value is very large and therefore not significant for fractal_dimension of the means. Additionally, the area_SD for the means class and worst class was very large and not tight like the other variables 
```{r}
